...
var xAxis = d3
  .axisTop(xScale)
  .ticks(6)
  .tickSize(-cellHeight * N, 0, 0);
var yAxis = d3
  .axisLeft(yScale)
  .ticks(6)
  .tickSize(-cellWidth * N, 0, 0);
...
d3.csv("cars.csv", dataPreprocessor).then(function (dataset) {
  ...
  cellEnter.append("g").attr("class", "brush").call(brush);
});
...
function brushmove(cell) {
  ...
  if (e) {
    ...
    svg.selectAll(".dot").classed("hidden", function (d) {
      return (
        e[0][0] > xScale(d[cell.x]) ||
        xScale(d[cell.x]) > e[1][0] ||
        e[0][1] > yScale(d[cell.y]) ||
        yScale(d[cell.y]) > e[1][1]
      );
    });
  }
}
